...
<div class="container">
    ...
    <form action="{{ route('discount.update', $discount->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="number" name="discount" class="form-control" id="discount" min="1" max="100" value="{{ $discount->discount }}" required>
        </div>

        <div class="mb-3">
            <label for="valid_until" class="form-label">Berlaku Hingga</label>
            <input type="date" name="valid_until" id="valid_until" class="form-control" value="{{ $discount->valid_until }}" required>
        </div>
        ...
        <div class="mb-3 d-none" id="min-purchase-container">
            ...
            <input type="number" name="min_qty" class="form-control" id="min_qty" value="{{ $discount->min_qty }}">
        </div>

        <div class="mb-3 d-none" id="min-price-container">
            <label for="min_price" class="form-label">Minimal Total Harga (Rp)</label>
            <input type="number" name="min_price" class="form-control" id="min_price" value="{{ $discount->min_price }}">
        </div>

        <div class="mb-3">
            <label for="needed_poin" class="form-label">Poin Dibutuhkan</label>
            <input type="number" name="needed_poin" class="form-control" id="needed_poin" value="{{ $discount->needed_poin }}">
        </div>
